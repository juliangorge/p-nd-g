...
void Andypolis:: showBuiltBuildings(){
    ...
    for(int i = 0; i < this->building_quantity; i++){
        if(this->buildings[i]->getQuantity() > 0){
            cout << "# " << this->buildings[i]->getName() << " (Construidos: " << this->buildings[i]->getQuantity() << "), Coordenadas: " << this->map->printCoordsByName(this->buildings[i]->getName()) << endl;
        }
    }
...
void Andypolis:: recolectByMaterial(string material, int j){
    for(int i = 0; i < this->material_quantity; i++){
        if(this->materials[i]->getName() == material){
...
void Andypolis:: colectResources(){
    for(int j = 0; j < this->building_quantity; j++){
        ...
        {
            case 'm':
                recolectByMaterial("piedra", j);
                break;
            case 'a':
                recolectByMaterial("madera", j);
                break;
            case 'f':
                recolectByMaterial("metal", j);
                break;
        }
